...
import { 
  useBelanjaProyekIn, useSaveBelanjaProyekIn, useDeleteBelanjaProyekIn,
  useBelanjaProyekOut, useSaveBelanjaProyekOut, useDeleteBelanjaProyekOut,
  usePurchaseOrders, useUploadFile, useCompany
} from '@/hooks/useData';
...
export default function BelanjaProyek() {
  const { data: pemasukan = [], isLoading: loadIn } = useBelanjaProyekIn();
  const { data: pengeluaran = [], isLoading: loadOut } = useBelanjaProyekOut();
  const { data: pos = [], isLoading: loadPO } = usePurchaseOrders();
  const saveIn = useSaveBelanjaProyekIn();
  const delIn = useDeleteBelanjaProyekIn();
  const saveOut = useSaveBelanjaProyekOut();
  const delOut = useDeleteBelanjaProyekOut();
  const uploadFile = useUploadFile();
  const { data: company } = useCompany();

  const isLoading = loadIn || loadOut || loadPO;
  ...
  const [poOutId, setPoOutId] = useState('');
  ...
  const filteredOut = useMemo(() => {
    return pengeluaran.map(item => {
      const po = pos.find(p => p.id === item.po_out_id);
      return { ...item, po_out_number: po?.po_number || '-' };
    }).filter(item => {
      const matchSearch = item.keterangan?.toLowerCase().includes(searchTerm.toLowerCase()) || item.po_out_number?.toLowerCase().includes(searchTerm.toLowerCase());
      const matchDate = (!dateFrom || item.tanggal >= dateFrom) && (!dateTo || item.tanggal <= dateTo);
      return matchSearch && matchDate;
    }).sort((a, b) => new Date(b.tanggal).getTime() - new Date(a.tanggal).getTime());
  }, [pengeluaran, pos, searchTerm, dateFrom, dateTo]);
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="no-print space-y-6">
        <PageHeader 
          ...
          subtitle="Kelola pemasukan dan pengeluaran terkait proyek"
        />
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            <input 
              type="text"
              placeholder="Cari keterangan / PO..."
              value={searchTerm}
          ...
          <div className="border border-gray-100 rounded-xl overflow-hidden flex flex-col shadow-sm">
            ...
            <div className="overflow-x-auto min-h-[300px]">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/50">
                    <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Tgl / PO</th>
                    <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Keterangan</th>
                    <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Nominal</th>
                    <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase text-center">Bukti</th>
                    <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase text-right">Aksi</th>
                    ...
                    paginatedOut.map((item: any) => (
                      <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3">
                          <div className="text-sm font-medium text-gray-900 whitespace-nowrap">{formatDate(item.tanggal)}</div>
                          <div className="text-xs text-gray-500 mt-0.5 whitespace-nowrap">PO: {item.po_out_number || '-'}</div>
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-600 line-clamp-2">{item.keterangan || '-'}</td>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4">
            ...
            <form onSubmit={handleSaveOut} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Pilih PO Out (Opsional)</label>
                <select 
                  value={poOutId}
                  onChange={e => setPoOutId(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                >
                ...
                <div className="px-10 pt-8 pb-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      <span className="inline-block bg-blue-800 text-white font-bold text-[8pt] uppercase tracking-widest px-3 py-1.5 rounded">Belanja Proyek</span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <table className="w-full text-left border-collapse border border-black text-sm">
                      <thead className="bg-blue-900 text-white">
                        <tr>
                          <th className="py-2.5 px-3 border-x border-black font-semibold w-[15%]">Tanggal</th>
                          <th className="py-2.5 px-3 border-x border-black font-semibold w-[15%]">Ref PO</th>
                          <th className="py-2.5 px-3 border-x border-black font-semibold">Keterangan</th>
                          <th className="py-2.5 px-3 border-x border-black font-semibold text-center w-[20%]">Dokumen</th>
                          <th className="py-2.5 px-3 border-x border-black font-semibold text-right w-[15%]">Nominal</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-black">
                        {filteredOut.length === 0 ? (
                          <tr><td colSpan={5} className="py-4 text-center text-gray-500 italic">Tidak ada pengeluaran.</td></tr>
                        ) : (
                          filteredOut.map((item: any) => (
                            <tr key={item.id} className="bg-white">
                              <td className="py-3 px-3 border-x border-black align-top">{formatDate(item.tanggal)}</td>
                              <td className="py-3 px-3 border-x border-black align-top font-medium">{item.po_out_number || '-'}</td>
                              <td className="py-3 px-3 border-x border-black align-top">{item.keterangan || '-'}</td>
                              <td className="py-3 px-3 border-x border-black align-middle text-center">
                                {item.bukti_foto ? (
                      ...
                      <tfoot className="border-t border-black bg-gray-50 font-semibold break-inside-avoid">
                        <tr>
                          <td colSpan={4} className="py-2.5 px-3 border-x border-black text-right">TOTAL PENGELUARAN</td>
