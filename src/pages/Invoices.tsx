...
export default function Invoices() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const safeNum = String(invoiceNumber).replace(/\//g, '_');
      ...
      generateAndUploadPdf({
        ...
        saveInvoice.mutate({ ...data, dokumen: pdfUrl });
      }).catch(e => console.warn('Invoice PDF generation failed:', e));
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const cCode = customers.find(c => c.id === inv.customer_id)?.code || '';
      const safeNum = String(invoiceNumber).replace(/\//g, '_');
      const pdfFilename = `PT MPA_${safeNum}_${cCode}.pdf`;
      generateAndUploadPdf({
        type: 'invoice',
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl mx-4 overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="p-6 overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Kolom Kiri: Info & Input Tanggal/Catatan */}
              <div className="space-y-4">
                <div className="bg-green-50 border border-green-100 rounded-xl p-3.5 text-sm text-green-900 space-y-1.5">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-green-700 font-medium">No. Invoice:</span>
                    <span className="font-semibold text-green-950">{inv?.invoice_number}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-green-700 font-medium">Customer:</span>
                    <span className="font-semibold text-green-950 truncate max-w-[200px]" title={custName}>{custName}</span>
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1.5">Catatan Pembayaran (opsional)</label>
                  <textarea
                    value={paymentNote}
                    onChange={e => setPaymentNote(e.target.value)}
                    rows={4}
                    className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none"
                    placeholder="Catatan pembayaran / keterangan..."
                  />
                </div>
              </div>

              {/* Kolom Kanan: Bukti Transfer & Upload File */}
              <div className="flex flex-col">
                ...
                {inv?.payment_proof_url ? (
                  <div className="mb-3 p-3 bg-emerald-50/70 border border-emerald-200 rounded-xl flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <div className="bg-white border border-emerald-100 rounded-lg p-2 flex items-center justify-center overflow-hidden max-h-40">
                        <img
                    ...
                    <p className="text-[10.5px] text-gray-500 mt-2">
                      Pilih file di bawah hanya jika ingin mengganti bukti transfer di atas.
                    </p>
                  </div>
                ) : (
                  <div className="mb-3 p-4 bg-gray-50 border border-gray-200 rounded-xl text-center flex-1 flex flex-col items-center justify-center text-gray-400">
                    <FileText className="w-8 h-8 mb-1 text-gray-300" />
                ...
                <div>
                  <input
                    ...
                    className="w-full px-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 file:mr-3 file:py-1 file:px-2.5 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                    accept="image/*,.pdf"
                  />
                  {paymentFile && (
                    <p className="text-xs text-blue-600 mt-1 font-medium truncate">File baru: {paymentFile.name}</p>
                  )}
                </div>
              </div>
            </div>
