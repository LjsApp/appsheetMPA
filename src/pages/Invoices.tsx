...
import { Loader2, Receipt, Plus, X, Trash2, Printer, Pencil, SendHorizonal, BadgeCheck, HandCoins, FileText, Upload } from 'lucide-react';
...
export default function Invoices() {
  ...
  const handleSyncPdf = async (inv: any) => {
    setGeneratingPdfId(inv.id);
    try {
      const cCode = customers.find(c => c.id === inv.customer_id)?.code || '';
      const safeNum = String(inv.invoice_number || inv.id).replace(/\//g, '_');
      ...
      await saveInvoice.mutateAsync({ ...inv, dokumen: url });
      toast.success('PDF Invoice berhasil diupload ke Google Drive');
    } catch (e: any) {
