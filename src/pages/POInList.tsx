...
import { Trash2, Loader2, FileText, Package, Search, Plus, Download } from 'lucide-react';
import { PageHeader, Button } from '@/components/ui';
import { usePoIns, useDeletePoIn } from '@/hooks/useData';
import { formatDate } from '@/lib/utils';
import type { POIn } from '@/types';

export default function POInList() {
  const { data: poIns = [], isLoading } = usePoIns();
            ...
            <table className="min-w-full divide-y divide-gray-100 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500">PIC</th>
                  ...
                  filtered.map(p => {
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-gray-700 whitespace-nowrap">{p.pic_name || '—'}</td>
                        ...
                        <td className="px-4 py-3">
                          <button
                            ...
                            className={`p-1.5 rounded transition-colors ${isConfirming ? 'bg-red-100 text-red-600' : 'text-gray-400 hover:text-red-600 hover:bg-red-50'}`}
                            title={isConfirming ? 'Klik lagi untuk konfirmasi hapus' : 'Hapus'}
                          >
                            {deletePoIn.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                          </button>
                        </td>
                      </tr>
                    );
